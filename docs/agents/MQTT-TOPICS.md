...
| `heatingassistant/{instance_id}/status` | Yes | App | HA bridge/operator | Runtime health and current room summary. Also triggers a Restart required **repair** check on the thin integration. |
| `homeassistant/update/heatingassistant_restart/config` | Yes | App | HA MQTT discovery | Empty retained payload (tombstone). Restart required is a Settings repair, not an Update entity. |
