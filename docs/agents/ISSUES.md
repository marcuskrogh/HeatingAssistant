...
|-----|------|-------|--------|--------|----------|------|
| SWD-432 | Task | [Sandbox] Room-view temperature forecast oscillates while planned power is smooth | To Do | — | docs/agents/SANDBOX-forecast-oscillation.md | `/implement SWD-432` — GHI `None` must use the cloud/clear path, not `ghi_now` |
...
## Log
- 2026-08-22 — `/sandbox` SWD-432 iteration 3: forced screenshot U is smooth in T (2 flips); interleaved GHI `None` → `ghi_now` reproduces ~1.94 K / 78 flips with that same U. Integrator vs RK45 17 mK. Artifact `docs/agents/SANDBOX-forecast-oscillation.md`. Next `/implement SWD-432`.
