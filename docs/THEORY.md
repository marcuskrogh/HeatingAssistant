...
The watt series after that intensity step is then passed through a first-order low-pass $q^+ = \alpha q_{\mathrm{inst}} + (1-\alpha) q^-$ with $\alpha = 1 - e^{-\Delta t / \tau}$.  The time constant $\tau$ is site-wide (`solar_gain_smoothing_tau_s`, default $1800\,\mathrm{s}$; Environment shows it in minutes).  $\tau = 0$ is identity.  Cloud-cover and GHI forecasts still choose the instantaneous intensity per step; the filter only stops those watts from jumping in a single control sample.  The first observation seeds the filter.  Live cycles keep the filtered NOW so the next cycle continues from there, and that NOW state is written to App runtime state so a restart (or a controller rebuild) continues the lag instead of reseeding.  The horizon is the causal walk from that state.
