...
|-------|------|------|------------|--------|-------|
| 1 | Discriminate: id_history JSONL missing rows vs horizon load ignoring disk | research | — | To Do | [SWD-319](https://marcusknielsen.atlassian.net/browse/SWD-319) |
...
- Whether silent `ID history store: append failed` occurred
...
`/research SWD-319` — discriminate JSONL missing rows vs horizon load ignoring disk (frontier by Order). Parallel unblocked: `/define SWD-320`.
