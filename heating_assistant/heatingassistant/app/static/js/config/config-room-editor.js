...
import { LANDING_CARDS } from './config-landing.js?v=124';
import {
  ROOM_SIZE_PRESETS,
  HOUSE_AGE_PRESETS,
  nearestPreset,
} from './config-presets.js?v=124';
import {
  el,
  schedulePanelNav,
  sectionCard,
...
function renderRoomEditor(container, connection, hass, idxParam) {
  ...
  connection.getModelConfig().then((cfg) => {
    ...
    envAdv.appendChild(paramGrid(
      ...
      numberField(room, 'facade_solar_share', 'Facade solar share', { step: 0.05, min: 0, max: 1, hint: 'Sol-air share on the room air node (0 = off).' }),
