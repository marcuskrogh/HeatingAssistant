...
a convex QP is solved via LinearisedContinuousMPC from the ``mbc`` toolbox.
...
HeatingMPCController
    ...
    LinearisedContinuousMPC, adds solar/outdoor forecasting, applies
...
class _InnovationEKF(ContinuousDiscreteEKF):
    ...
    LinearisedContinuousMPC calls ``estimator.step(y, u_prev, d_prev, p, t)``
...
class HeatingMPCController:
    ...
    LinearisedContinuousMPC, then provides the coordinator-facing API:

      actions = controller.compute(outdoor_temp, solar_gains, now, outdoor_forecast)

    The control loop at each step:
      1. LinearisedContinuousMPC.step(): runs EKF predict+update, linearises
    ...
    def __init__(
        ...
        # ── MPC controller ─────────────────────────────────────────────
        self._mpc = StandardLinearisedContinuousMPC(
            ...
            rho_lin=rho_lin,
            y_offset=y_offset,
            penalise_absolute_input=True,
            state_projection=lambda x, n=n_x_ctrl: np.asarray(x, dtype=float).ravel()[:n],
        )
    ...
    def _set_mpc_operating_point(
        ...
        x_ss = self._mpc.x_ref.copy()
        x_ss[n:nx_phys] = model.wall_equilibrium(x_ss[:n], float(d_now[0]))
        u_ss = model.compute_u_eq(x_ss, d_now, p, t)
