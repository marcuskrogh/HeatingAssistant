 ...
 * clicks are consumed locally (stopPropagation + debounced HA service call)
 * so they never trigger navigation.  Navigation is also blocked while a
 * commit timer is still pending.
 */
...
function buildScheduleHtml(schedData) {
...
export function createRoomClimateTile(room, state, hass, scheduleData) {
  ...
  function paint() {
    ...
    const html = buildScheduleHtml(schedData);
    if (html) {
